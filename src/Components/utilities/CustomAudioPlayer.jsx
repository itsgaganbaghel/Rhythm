...
const CustomAudioPlayer = ({ tracks }) => {
    ...
    let { songIndex, isPlaying, setIsPlaying } = useContext(addAlbumContext);
    ...
    const increaseVolume = () => {
        setVolume((prevVolume) => (prevVolume < 1 ? Number((prevVolume + 0.1).toFixed(1)) : 1));
    };

    const decreaseVolume = () => {
        setVolume((prevVolume) => (prevVolume > 0 ? Number((prevVolume - 0.1).toFixed(1)) : 0));
    };
  ...
  `;
    ...
    return (
        <div className="flex  max-h-[15vh] min-h-[10vh] items-center  bg-secondary shadow-2xl text-white w-[100vw] relative overflow-y-hidden px-4">

            <div className='min-w-[15%] h-full flex justify-start items-center gap-4   '>

                <img src={image} alt={`Artwork for ${title}`} className={`w-12 h-12 rounded-full  ${isPlaying == true && "custom-spin"}`} />
            ...
            <div className='w-[70%] h-full  '>
                <div className="w-full flex items-center justify-center gap-6   ">
                    ...
                    <button onClick={handlePrevTrack} className="text-gray-500 text-xl hover:text-white focus:outline-none">
                        <IoPlayBackSharp />
                    </button>
                    <button onClick={handlePlayPause} className="text-gray-500 hover:text-white focus:outline-none text-xl">
                        {isPlaying ? <FaPause /> : <IoPlaySharp />}
                    </button>
                    <button onClick={handleNextTrack} className="text-gray-500 text-xl hover:text-white focus:outline-none">
                        <IoPlayForwardSharp />
                    </button>
                ...
                <div className="w-full flex justify-center items-center gap-6 text-gray-400  ">
                    ...
                    />
                    <p className='text-xl pb-1'>{duration ? formatTime(duration) : "--/--"}</p>
                </div>
            </div>

            {/* Volume Controls */}
            <div className="flex justify-end  min-w-[15%] max-w-[15%]   ">

                <button
                    onClick={decreaseVolume}
                    className={`text-gray-500 hover:text-white focus:outline-none  p-2 rounded-full bg-primary ${volume === 0 ? "opacity-50 cursor-not-allowed" : ""}`}
                    disabled={volume === 0}
                >
                    <FaVolumeDown />
                </button>
                ...
                <button
                    ...
                    className={`text-gray-500 hover:text-white focus:outline-none p-2 rounded-full bg-primary ${volume === 1 ? "opacity-50 cursor-not-allowed" : ""}`}
                    disabled={volume === 1}
                >
                    <FaVolumeUp />
                </button>
